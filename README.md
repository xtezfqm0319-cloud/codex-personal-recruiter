...
**你负责找人和做决定，整理文件、归档材料、筛选比较和准备报告这些重复工作，交给 Codex。**

这是一个直接在 Codex 中使用的本地个人招聘工作台。它会陪你完成从建岗位、筛简历、排候选人、准备面试，到终面决策和人才归档的完整流程。它不只是把简历总结一遍，而是真的替你整理材料、维护岗位进展，并逐渐记住你真正看重什么。

这个项目适合独立负责招聘的 HR、招聘负责人、HRBP，以及需要深度参与选人的业务负责人。它不是一套需要部署和维护的 ATS，也不是面向所有公司的通用 SaaS；你可以把它 fork 下来，慢慢调成只服务你和你公司的个人招聘工作台。

> [!IMPORTANT]
> **不用接模型 API，不用配置 API Key，也没有单独的 API 账单。** 只要已有包含 Codex 的 ChatGPT 套餐（例如 Plus），就可以在套餐额度内直接使用。项目本身不调用外部模型 API，也不需要服务器、数据库或另一套招聘系统。套餐和用量以 [Codex 官方说明](https://learn.chatgpt.com/docs/pricing) 为准。
...
2. 对 Codex 说：“处理新简历，告诉我这批人应该先约谁。”
3. Codex 会读取岗位要求并完成分析；本地脚本负责识别、重命名、移动、建档、更新索引和校验。
...
面试纪要也是同样的用法：全部放进统一入口，Codex 会识别候选人、岗位和轮次，归入对应候选人档案，生成面试报告并更新流程状态。候选人流程结束后，完整档案会进入历史人才库，以后新开相似岗位时还能重新找到。
...
| 面试结束 | 区分面试官原话、Codex 分析和你的正式结论 | 可追溯的独立面试报告和流程状态 |
...
- **不用学一套新系统。** Codex 对话就是入口，你说目标，它负责读取材料、调用对应 Skill 并把正式结果落盘。
...
1. 在 Codex 中打开这个项目，根据自己的公司填写 `00_公司认知/`。
