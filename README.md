...
> [!IMPORTANT]
> **当前版本 v0.3.0 更偏向“懂你的个人招聘助理”。** 除了支持图片版、扫描版和混合内容 PDF，它还会记住你明确确认的招聘偏好、横向比较同岗候选人、准备针对性面试问题，并给出更清楚的下一步建议。

## 最舒服的使用方式

不需要记命令，也不需要关心文件怎么移动。直接在 Codex 里说你想解决的问题即可，例如：
...
Codex 会优先给结论、候选人之间的相对差异和下一步动作。文件整理、索引重建等例行操作只在失败或你主动询问时说明。
...
- 今日简报、候选人比较和面试准备是可重建的辅助视图，准确度取决于主档案中已有材料的完整度。
